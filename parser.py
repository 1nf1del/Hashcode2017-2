...
def parsing(path):
    ...
    for i in range(E):
        ...
        for j in range(cacheServersNumber):
            ...
            if idServer in globalCacheServers:
                globalCacheServers[idServer].endPoints[idE]=({'endpoint':idE, 'latency' : latencyServer})
            else:
                globalCacheServers[idServer] = o.CacheServer(idServer,X)
                globalCacheServers[idServer].endPoints[idE]=({'endpoint':idE, 'latency' : latencyServer})
